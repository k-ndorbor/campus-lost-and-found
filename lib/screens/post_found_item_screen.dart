...
class _PostFoundItemScreenState extends State<PostFoundItemScreen> {
  ...
  Future<String?> _uploadImage() async {
    ...
    try {
      ...
      final fileName = 'found_${DateTime.now().millisecondsSinceEpoch}${path.extension(_imageFile!.path)}';
  ...
  Future<void> _submitItem() async {
    ...
    try {
      ...
      _showError('Failed to post item');
    } finally {
      if (mounted) setState(() => _isLoading = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Post Found Item', style: TextStyle(color: Colors.white)),
        backgroundColor: const Color(0xFF1A237E),
        elevation: 0,
        leading: IconButton(
              ...
              child: Form(
                ...
                child: ListView(
                  children: [
                        ...
                        child: _imageFile == null
                            ? const Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 8),
                                  Text('Add Photo', style: TextStyle(color: Colors.white70)),
                                ],
                              )
                            : null,
                      ),
                    ...
                    const SizedBox(height: 10),
                    _buildTextField(_locationController, 'Location Found'),
                    const SizedBox(height: 10),
                    _buildDateField(),
                    const SizedBox(height: 10),
